...
--Câu 1: Liệt kê danh sách các hóa đơn (SalesOrderID) lặp trong tháng 6 năm 2008 có tổng tiền >70000, thông tin gồm SalesOrderID, Orderdate, SubTotal, trong đó SubTotal =sum(OrderQty*UnitPrice). 
...
--Câu 2: Đếm tổng số khách hàng và tổng tiền của những khách hàng thuộc các quốc gia có mã vùng là US (lấy thông tin từ các bảng SalesTerritory, Sales.Customer, Sales.SalesOrderHeader, Sales.SalesOrderDetail). 
...
--Câu 3: Tính tổng trị giá của những hóa đơn với Mã theo dõi giao hàng (CarrierTrackingNumber) có 3 ký tự đầu là 4BD, thông tin bao gồm SalesOrderID, CarrierTrackingNumber, SubTotal=sum(OrderQty*UnitPrice)  
...
--Câu 4: Liệt kê các sản phẩm (product)có đơn giá (unitPrice)<25 và số lượng bán trung bình >5, thông tin gồm ProductID, name, AverageofQty
...
--Câu 5: Liệt kê các công việc (JobTitle) có tổng số nhân viên >20 người, thông tin gồm JobTitle, countofPerson=count(*) 
...
--thông tin gồm BusinessEntityID, Vendor_name, ProductID, sumofQty, SubTotal (sử dụng các bảng [Purchasing].[Vendor] [Purchasing].[PurchaseOrderHeader] và [Purchasing].[PurchaseOrderDetail])
...
--Câu 8: Liệt kê danh sách các khách hàng có trên 25 hóa đơn đặt hàng từ năm 2007 đến 2008, thông tin gồm mã khách (PersonID) , họ tên (FirstName +' '+ LastName as fullname), Số hóa đơn (CountOfOrders)--
...
--Câu 10: Liệt kê những phòng ban có lương (Rate: lương theo giờ) trung bình >30, thông tin gồm Mã phòng ban (DepartmentID), tên phòng ban (name), Lương trung bình (AvgofRate). 
...
--Câu 3: Hiển thị thông tin của khách hàng có số đơn đặt hàng nhiều nhất, thông tin gồm: CustomerID, Name, CountofOrder--
...
having COUNT(*)>=all(select COUNT(*)
					 from Sales.Customer c join Sales.SalesOrderHeader h on c.CustomerID=h.CustomerID
					 group by c.CustomerID)	
--Câu 4: Liệt kê các sản phẩm (ProductID, Name) thuộc mô hình sản phẩm áo dài tay với tên bắt đầu với “Long-Sleeve Logo Jersey”, dùng phép IN và EXISTS, (sử dụng bảng Production.Product và Production.ProductModel)--	
select ProductID, Name
from Production.Product 
...
--Câu 5: Tìm các mô sản phẩm (ProductModelID) mà giá niêm yết (list price) tối đa cao hơn giá trung bình của tất cả các mô hình--	
...
--Câu 9: Liệt kê các nhân viên không lập hóa đơn từ sau ngày 1/5/2008, thông tin gồm EmployeeID, FirstName, LastName (dữ liệu từ 2 bảng HR.Employees và Sales.Orders)--
...
--Câu 10. Liệt kê danh sách các khách hàng (customerID, name) có hóa đơn dặt hàng trong năm 2007 nhưng có hóa đơn đặt hàng trong năm 2008--
select [CustomerID]
from [Sales].[SalesOrderHeader]
...
--Câu 1) Tạo hai bảng mới trong cơ sở dữ liệu AdventureWorks2008 theo cấu trúc sau:
...
-- Câu 2) Dùng lệnh insert <tableName1> select <fieldList>from <TableName2>
...
--Câu 3) Tương tự câu 2, chèn 20 dòng dữ liệu cho bảng MyEmployee lấy dữ liệu từ 2 bảng [Person].[Person]và[HumanResources].[EmployeeDepartmentHistory]
